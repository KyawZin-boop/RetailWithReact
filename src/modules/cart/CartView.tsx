...
import { cartSlice, reduceItem } from "@/store/features/cartSlice";
import { MinusIcon, PlusIcon, ShoppingBagIcon, Table } from "lucide-react";
                        ...
                        {CartItems.map((item, index) => (<TableRow key="index + 1">
                            ...
                            <TableCell className="text-center">
                                <div className="flex justify-evenly items-center">
                                    ...
                                    sonClick="cartStore.IncreaseItem(item)">
                                        <PlusIcon />
                                    </span>
                                </div>
                            </TableCell>
                            <TableCell className="text-center">$ {{ item.price }}</TableCell>
                            <TableCell className="flex gap-2 text-white flex justify-center">
                                <button className="bg-red-500 rounded-md p-2 px-4"
                                    onClick="cartStore.removeFromCart(item)">Remove</button>
                            </TableCell>
                        </TableRow>))}
                        {/* <TableRow key="index + 1">
                            ...
                            <TableCell className="text-center">
                                <div className="flex justify-evenly items-center">
                                    <span
                                        className=" rounded-full px-2 font-semibold text-red-600 cursor-pointer hover:bg-gray-200 select-none"
                                        onClick="cartStore.decreaseItem(item)">
                                        <MinusIcon />
                                    </span>
                                    <span className="w-[30px]">{{ item.quantity }}</span>
                                    <span
